...
def save_memory(conversation_history, role_system):
    ...
    try:
        data = {
            ...
            "last_update": str(os.path.getmtime(MEMORY_FILE)) if os.path.exists(MEMORY_FILE) else ""
        }
...
def roles(role_name):
    role_dict = {
        "保镖": """你所有的回答都要符合保镖的职业特点，说话要专业、警惕，注重个人保护，语气可能更简洁、专业。""",
        "保安": "你所有的回答都要符合保安的职业特点，说话要正式、严谨，注重维护秩序和公共安全，语气可能比较官方。"
    }
    return role_dict.get(role_name, "你是一个普通的人，没有特殊角色。")

# 【系统角色设定】
# 定义AI的角色和性格特征
role_name = "保镖"  # 可以改为"保安"或"保镖"
role_system = roles(role_name)
